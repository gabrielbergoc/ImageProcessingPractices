...
public class CodeClass {
	...
	*/
	public static String whatTime(ImageAccess input) {
		...
		time = "Time: " + (int)Math.floor(((double)hour / (double)nbOfAngles) * 12) + ":" + Math.round(((double)minute / (double)nbOfAngles) * 60);
	...
	*/
	private static double getInterpolatedPixelCubicSpline(ImageAccess coef, double x, double y) {
		// double v = InterpolationSolution.getInterpolatedPixelCubicSpline(coef, x, y);
		
        double[][] neighbor = new double[4][4];
        int m = (int)Math.floor(x);
        int n = (int)Math.floor(y);
        
		imageAccess.getNeighborhood(m, n, neighbor);
        v = getSampleCubicSpline(x - m, y - n, neighbor);
		return v;
	}
	...
	*/
	static private double getSampleCubicSpline(double x, double y, double neighbor[][]) {
		...
		for (int i = 0; i < 4; ++i) {
            for (int j = 0; j < 4; ++j) {
                sum += array[j][i] * cubicSplineRow[j] * cubicSplineCol[i];
            }
        }
