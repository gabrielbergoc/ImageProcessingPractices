...
public class ButterHighPass_ extends PlugInFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (label == null)
			return;
		new ButterHighPassRunner(label);
	}
...
class ButterHighPassRunner extends Thread {
	private String command;
	private ImagePlus imp;
	private ImageProcessor ip;

	ButterHighPassRunner(String command) {
		super(command);
	...
	void runCommand(String command) {
		...
		for (i = 0; i < 256; i++) {
			for (j = 0; j < 256; j++) {
				...
				double value = 1 / (1 + (radius / r));
				im.putPixel(i, j, value);
			}
		}

		im.show("Butterworth High Pass " + command);
	}
}
